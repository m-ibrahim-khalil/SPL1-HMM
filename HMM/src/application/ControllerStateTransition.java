...
public class ControllerStateTransition implements Initializable{
	...
	public void buttonProcess (ActionEvent event) throws IOException
	{
		...
		for(Object o: list) {
			...
			Variable.transitionProbability[i][j] = Double.parseDouble(((TextField)((HBox) o).getChildren().get(1)).getText().toString());
			System.out.println(Variable.transitionProbability[i][j] + " " + i + " " + j );
			if (j == ((Variable.nState)-1))
			{
				//System.out.println(Variable.nState);
				j = 0;
				i++;
			}
